...
interface DeployFlowProps {
  onSafeCreated: (safe: SavedSafe) => void;
}

export default function DeployFlow({ onSafeCreated }: DeployFlowProps) {
  ...
  const handleDeploySafe = async () => {
    if (!signerAddress || !credential) return;
      ...
      {safeAddress && (
        <>
          ...
          <div style={stepStyle}>
            ...
            <button onClick={handleSign} disabled={balance === 0n || statuses.sign === 'loading' || statuses.sign === 'done'}>
              Sign with Passkey
            </button>
